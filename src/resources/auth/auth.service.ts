...
export class AuthService {
  ...
  async registerUser(data: RegistrationDTO) {
    ...
    const ACCOUNT_ACTIVATION_ENABLED =
      this.configService.get('ACCOUNT_ACTIVATION_ENABLED') === 'true';
  ...
  async refreshTokens(refreshToken: string) {
    ...
    const payload: any = await this.securityService.verifyJwt(
      refreshToken,
  ...
  async requestResetPassword(data: RequestResetPasswordDTO) {
    ...
    const PASSWORD_RESET_TOKEN_LENGTH =
      +this.configService.get('PASSWORD_RESET_TOKEN_LENGTH') || 32;
    const PASSWORD_RESET_CODE_LENGTH =
      +this.configService.get('PASSWORD_RESET_CODE_LENGTH') || 6;
    const PASSWORD_RESET_LIMIT_ENABLED =
      this.configService.get('PASSWORD_RESET_LIMIT_ENABLED') === 'true';
    const PASSWORD_RESET_TOKEN_EXPIRE =
      +this.configService.get('PASSWORD_RESET_TOKEN_EXPIRE') || 300;
    ...
    if (PASSWORD_RESET_LIMIT_ENABLED) {
      ...
      if (pendingRequest) {
        ...
        const remainingTime = Math.floor(
          (pendingRequest.expires_at.getTime() - Date.now()) / 1000,
        );

        throw new HttpException(
          `Un email a déjà été envoyé. Veuillez patienter ${remainingTime} secondes avant de pouvoir en demander un autre.`,
          HttpStatus.TOO_MANY_REQUESTS,
        );
      }
    }

    const passwordResetToken = this.randomService.randomToken(
      PASSWORD_RESET_TOKEN_LENGTH,
    );
    const passwordResetCode = this.randomService.randomOtp(
      PASSWORD_RESET_CODE_LENGTH,
    );
    const passwordResetExpireAt =
      PASSWORD_RESET_TOKEN_EXPIRE !== -1
        ? new Date(Date.now() + PASSWORD_RESET_TOKEN_EXPIRE * 1000)
        : null;
  ...
  async resetPassword(data: ResetPasswordDTO) {
    ...
    if (!request.confirmed_at) {
      throw new HttpException(
        'Veuillez confirmer votre demande de réinitialisation de mot de passe',
        401,
      );
    }
  ...
  async createAccessTokenAndRefreshToken(userId) {
    ...
    const userData: UserData = {
      ...
      lastname: user.noms,
      profile: user.profile?.libelle as Profile,
      email: user.email,
    ...
    // Générer les tokens de manière asynchrone
    const accessToken = await this.securityService.signJwt(
      payloadAT,
      this.configService.get('ACCESS_TOKEN_SECRET'),
      this.configService.get('ACCESS_TOKEN_EXPIRE'),
    );

    const refreshToken = await this.securityService.signJwt(
      payloadRT,
      this.configService.get('REFRESH_TOKEN_SECRET'),
  ...
  async updateAutoLoginToken(userId) {
    const AUTO_LOGIN_TOKEN_LENGTH =
      +this.configService.get('AUTO_LOGIN_TOKEN_LENGTH') || 16;

    const autoLoginToken = this.randomService.randomToken(
      AUTO_LOGIN_TOKEN_LENGTH,
    );
  ...
  async updateActivationToken(userId) {
    ...
    if (!user.mail_verified_at) {
      const ACTIVATION_TOKEN_LENGTH =
        +this.configService.get('ACTIVATION_TOKEN_LENGTH') || 16;
      const ACTIVATION_CODE_LENGTH =
        +this.configService.get('ACTIVATION_CODE_LENGTH') || 6;
      const ACTIVATION_TOKEN_EXPIRE =
        +this.configService.get('ACTIVATION_TOKEN_EXPIRE') || 300;

      const activationToken = this.randomService.randomToken(
        ACTIVATION_TOKEN_LENGTH,
      );
      const activationCode = this.randomService.randomOtp(
        ACTIVATION_CODE_LENGTH,
      );
      const activationExpireAt =
        ACTIVATION_TOKEN_EXPIRE !== -1
          ? new Date(Date.now() + ACTIVATION_TOKEN_EXPIRE * 1000)
          : null;
