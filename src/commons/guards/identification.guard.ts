...
export class IdentificationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Verify token JWT de manière asynchrone
    const resultJwt = await this.securityService.verifyJwt(
      token,
    ...
    request.extended_audit = [Profile.ADMIN, Profile.SUPER_ADMIN].includes(
      userData.profile,
    );
