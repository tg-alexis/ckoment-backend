  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const key =
      this.reflector.get<string>(
        'verify-ownership-key',
        context.getHandler(),
      ) || 'id';
    const table = this.reflector.get<string>(
      'verify-ownership-table',
      context.getHandler(),
    );
    const tableProperty = this.reflector.get<string>(
      'verify-ownership-tableProperty',
      context.getHandler(),
    );
    const propertyPath = this.reflector.get<string>(
      'verify-ownership-propertyPath',
      context.getHandler(),
    );
